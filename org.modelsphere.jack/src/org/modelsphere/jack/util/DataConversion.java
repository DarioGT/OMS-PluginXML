 ...
 * this sole class.
 * 
 * TD-- : short[] and long[] use the 'C-String' approach, ie use a terminator to end each element. A
...
public final class DataConversion {
    ...
    private String fromObjectsToString(Object[] array) {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (array[i] == null)
                continue;
            //TD-- SrType may be contained in a SrType ... there should be a
    ...
    public String fromSrTypeToString(SrType srtype) {
        ...
        while (c != null && c != SrType.class) {
            ...
            for (int i = 0; fields != null && i < fields.length; i++) {
                ...
                if (value == null) {
                    value = ""; //TD-- maybe better to skip ?????
                }

                xml.write(fields[i].getName());
                xml.write(XMLConstants.ATTR_NAME_VALUE_SEPARATOR); // write '='
                //TD-- SrType may be contained in a SrType ... there should be
