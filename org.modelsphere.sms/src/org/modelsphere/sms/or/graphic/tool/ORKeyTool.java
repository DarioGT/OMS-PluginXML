...
public class ORKeyTool extends KeyTool {
    ...
    protected void createDependency(GraphicComponent label) {
        if (label instanceof MultiplicityLabel) {
            MultiplicityLabel multilabel = (MultiplicityLabel) label;
            DbORAssociationEnd end = (DbORAssociationEnd) multilabel.getSemanticalObject();
            ...
            try {
                ...
                if (classifier instanceof DbORTable) {
                    ...
                    if (isERMode)
                        ...
                        tablebox = end.isFrontEnd() ? (ORTableBox) association.getNode1()
                    ...
                    if (end.getDependentConstraints().indexOf(toolKey) == -1)
                        end.addToDependentConstraints(toolKey);
                    else
                        end.removeFromDependentConstraints(toolKey);
                }
